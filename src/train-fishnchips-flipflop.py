...
config_filename = "configs/train-fishnchips-config-25H-5CNN-flipflop.json"
...
MODEL_SAVE_FILENAME = f"./trained_models/fishnchips_{config['D_MODEL']}_{config['CNN_BLOCKS']}CNN_{config['NUM_HEADS']}H_{config['ATTENTION_BLOCKS']}B_flipflop"
...
generator = AttentionFlipflopDataGenerator(read_ids, config['BATCH_SIZE'], config['STRIDE'], config['ENCODER_MAX_LENGTH'], config['DECODER_MAX_LENGTH'])
...
fish = FishNChips(
  ...
  output_dim=1 + (4 * 2) + 1 + 1, # PAD + ATCG + START + STOP
