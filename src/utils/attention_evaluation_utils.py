...
def evaluate_batch_ctc_combined(inp, model, batch_size, as_bases=True):
    ...
    for _ in range(model.pe_decoder_max_length):
        combined_mask = create_combined_mask(output)
        _, predictions = model(inp, output, False, combined_mask, use_cached_enc_output) # (batch_size, i + 1, vocab_size)
        use_cached_enc_output = True
